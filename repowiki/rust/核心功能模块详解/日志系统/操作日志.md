...
+find_all_operation_record(search, page, sort) Result<Vec<OperationRecordModel>>
+find_count_operation_record(search) Result<u64>
+creates_operation_record(inputs) Result<Vec<OperationRecordId>>
+delete_by_ids_operation_record(ids) Result<u64>
}
class UserService {
+create_user(data) Result<UserId>
+update_user(id, data) Result<()>
+delete_user(id) Result<()>
}
class MenuService {
+create_menu(data) Result<MenuId>
+update_menu(id, data) Result<()>
+delete_menu(id) Result<()>
}
